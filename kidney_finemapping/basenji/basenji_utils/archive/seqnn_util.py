...
class SeqNNModel(object):
    ...
    def build_grads(self, layers=[0], center=False):
        ...
        for ti in range(self.hp.num_targets):
            if center:
                grad_ti_op = tf.gradients(self.preds_train[:, ci:ci + 2, ti],
            ...
            for mi in range(mc_n):
                ...
                for ti in range(self.hp.num_targets):
                    ...
                    for lii in range(len(self.grad_layers)):
                        ...
                        # update mean
                        layer_grads[lii][ti] = self.running_mean(layer_grads_lii_ti1, layer_grads_ti_ei[lii],
                                                                 running_i + 1)

                        # update variance sum
                        if return_var:
                            layer_grads_var[lii][ti] = self.running_varsum(layer_grads_var[lii][ti],
