...
def snp_seq1_pos_shifts(snp, seq_len, left_len, genome_open):
    """ Produce a one hot coded sequences for a SNP.
...
def snp_seq1_ism(snp, seq_len, left_len, genome_open, use_alt=False):
    """ Produce a one hot coded sequences for a SNP.
    Regardless of the ref allele or alt allele in the SNP, this will use the allele in the reference genome
    as the reference allele and all other 3 possible alleles as the "alt alleles."
    ...
    # verify that ref allele matches ref sequence
    left_len_original = snp.original_pos - seq_start
    seq_ref = seq[left_len_original:left_len_original + len(snp.ref_allele)]
    ref_found = True
    swapped = False
    if seq_ref != snp.ref_allele:
        ...
        for alt_al in snp.alt_alleles:
            ...
            # grab reference sequence matching alt length
            seq_ref_alt = seq[left_len_original:left_len_original + len(alt_al)]
            if seq_ref_alt == alt_al:
                # found it!
                ref_found = True
                ...
                # remove alt allele and include ref allele
                seq = seq[:left_len_original] + snp.ref_allele + seq[left_len_original + len(alt_al):]
                break

    if not ref_found:
        assert False, f"Supplied ref allele was not found in reference genome for snp {snp.rsid}"

    if use_alt:
        if not swapped:
            # change sequence to use the SNP's alt_allele for ISM scores
            assert len(snp.alt_alleles) == 1, "Only one alt allele per variant supported"
            snp_alt = snp.alt_alleles[0]  # There should only be one alt allele here, so we flatten the list
            if seq_start <= snp.original_pos and snp.original_pos <= seq_end:  # Compare seq_start and seq_end to original_pos (1-based, inclusive)
                seq = seq[:left_len_original] + snp_alt + seq[left_len_original + len(snp.ref_allele):]
        else:
            # change sequence to use the SNP's alt_allele for ISM scores
            assert len(snp.alt_alleles) == 1, "Only one alt allele per variant supported"
            snp_ref = snp.ref_allele
            if seq_start <= snp.original_pos and snp.original_pos <= seq_end:
                seq = seq[:left_len_original] + snp_ref + seq[left_len_original + len(snp.alt_alleles[0]):]


    if seq_len < len(seq):
        # trim the sequence (if either alt or ref have length > 1)
        seq_trim = (len(seq) - seq_len) // 2
        seq = seq[seq_trim:seq_trim + seq_len]

    for alt_al in ['A', 'C', 'G', 'T']:
        # remove ref allele and include alt allele
        seq_alt = seq[:left_len] + alt_al + seq[left_len + 1:]

        # one hot code
        seq_vecs_alt, seq_alt = dna_length_1hot(seq_alt, seq_len)
        seq_vecs_list.append(seq_vecs_alt)

    return seq_vecs_list, seq
                ...
                if snps[-1].ref_allele != ref_snp:
                    if not flip_ref:
                        ...
                        if snps[-1].alt_alleles[0] == ref_snp:
                            print(f"Flipping alt and ref for {snps[-1].rsid}")
                            # flip alleles
                            snps[-1].flip_alleles()
...
class SNP:
    ...
    def __init__(self, vcf_line, pos2=False, original_pos=None):
        a = vcf_line.split()
        ...
        if original_pos:
            self.original_pos = int(original_pos)  # Keep track of original position for "fake" SNPs in ISM
