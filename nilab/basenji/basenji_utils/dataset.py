...
class SeqDataset:
    def __init__(self, data_dir, split_label, batch_size, shuffle_buffer=32,
                 seq_length_crop=None, mode=tf.estimator.ModeKeys.EVAL, tfr_pattern=None, from_tensor_slices=False):
        """Initialize basic parameters; run compute_stats; run make_dataset."""
        ...
        self.mode = mode
        self.tfr_pattern = tfr_pattern
        self.from_tensor_slices = from_tensor_slices
        ...
        self.crop_bp = data_stats['crop_bp']
        ...
        self.make_dataset(from_tensor_slices=self.from_tensor_slices)
    ...
    def make_dataset(self, cycle_length=4, from_tensor_slices=False):
        """Make Dataset w/ transformations."""

        # initialize dataset from TFRecords glob
        tfr_files = natsorted(glob.glob(self.tfr_path))
        if from_tensor_slices and tfr_files:
            print('Making dataset from tensor slices...')
    ...
    def numpy_sample(self, num_samples, return_inputs=True, return_outputs=True, step=1):
        """ Convert TFR inputs and/or outputs to numpy arrays."""
        with tf.name_scope('numpy'):
            # initialize dataset from TFRecords glob
            tfr_files = natsorted(glob.glob(self.tfr_path))
