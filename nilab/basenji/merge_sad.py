...
def main():
    ...
    parser.add_option('--vcf', dest='vcf',
                      ...
                      help='If set, combine SNP VCFs from SAD output to out directory.')
    ...
    assert len(sad_files) == options.num_files, 'Expected {} sad files for {} chromosomes, got {} files'.format(
        options.num_files, options.num_files, len(sad_files))
    ...
    sad_h5.create_dataset('ref_allele',
                          data=np.array([allele for sad in sads for allele in sad['ref_allele']], dtype='S'))
    sad_h5.create_dataset('alt_allele',
                          data=np.array([allele for sad in sads for allele in sad['alt_allele']], dtype='S'))
    ...
    if options.vcf:
        ...
        assert len(vcf_files) == options.num_files, 'Expected {} sad files for {} chromosomes, got {} files'.format(
            options.num_files, options.num_files, len(vcf_files))
