...
def main():
    ...
    parser.add_option('--use_alt', dest='use_alt',
                      default=False, action='store_true',
                      help='Use alt allele for computing ISM scores [Default: %default]')
    ...
    #   else:
    #     # read SNPs form VCF
    snps = bvcf.vcf_snps(vcf_file, validate_ref_fasta=options.genome_fasta)

    # Prepare scores_h5['seqs']
    _, _, _, seqs = bvcf.snps_seq1(snps, params_model['seq_length'], options.genome_fasta,
                                   return_seqs=True)  # skip alt allele seqs
    seqs = seqs[::2]

    snps_for_mut = bvcf.vcf_snps_ism(vcf_file, options.mut_up, options.mut_down)

    num_snps_for_mut = len(snps_for_mut)
    ...
    def snp_gen():
        for snp in snps_for_mut:
            for i in range(num_pos):
                # get SNP sequences
                snp_1hot_list = bvcf.snp_seq1_ism(snp, params_model['seq_length'], i, genome_open, options.use_alt)
    ...
    scores_h5.create_dataset('label',
                             data=np.array([snp.rsid for snp in snps], dtype='S'))  # Skip mut_len for the rsids
    scores_h5.create_dataset('seqs', dtype='bool',
                             shape=(num_seqs, options.mut_len, 4))

    # Insert into scores_h5['seqs']
    ...
    for si in range(num_seqs):
        # snp_seq1 gives _ _ V _ _ _, but ISM mutation window is _ _ V _
        seq_dna_mut = seqs[si][mut_start - 1:mut_end - 1]
        seq_1hot_mut = dna_io.dna_1hot(seq_dna_mut)
        scores_h5['seqs'][si, :, :] = seq_1hot_mut

    # Create alt_allele labels
    vcf_df = pd.read_table(vcf_file, names=['chr', 'pos', 'rsid', 'ref', 'alt'], comment='#')
    alt_alleles = vcf_df['alt']
    snp_alts = np.array(alt_alleles, 'S')
    scores_h5.create_dataset('alt_allele', data=snp_alts)
    ...
    sum_stat = np.zeros((num_snps_for_mut // options.mut_len, options.mut_len, 4, num_targets))  # Initialize sum_stat
    for si in range(num_snps_for_mut):  # num_snps_for_mut is actually num_snps * options.mut_len
        for posi in range(num_pos):
            for ni in range(4):
                sum_stat[si // options.mut_len, si % options.mut_len, ni, :] += preds_stream[
                    pi].squeeze()  # Sum over positions for sum_stat. One-hot encoding order should be ACGT...
                pi += 1

    # Subtract mean over nucleotides so that it can be stored in a float16 or maybe float32
    sum_stat -= sum_stat.mean(axis=2, keepdims=True)

    # Divide out num_pos so that we get average over SAD track.
    sum_stat = sum_stat / num_pos
    ...
    # Finally, write the ISM sum preds to dataset of shape (num_seqs, options.mut_len, 4, num_targets)
    scores_h5.create_dataset('sum', dtype='float32',
                             data=sum_stat)  # num_seqs should be same as num_snps_for_mut // options.mut_len
    scores_h5.close()
